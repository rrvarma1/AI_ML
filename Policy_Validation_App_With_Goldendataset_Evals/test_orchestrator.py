...
def test_pvc_gd_027_routes_on_expected_compliance_only(tmp_path):
    text = """Borrower Name: GreenLeaf Packaging Private Limited
Deal Type: New secured term loan
Loan Amount: USD 50000000
Interest Rate: 10.25%
Term Years: 5
Term Months: 60
Estimated DSCR: 1.42x
Debt-to-Equity Ratio: 1.8x
Total Debt: INR 63000000
Tangible Net Worth: INR 35000000
Current Ratio: 1.35x
Revenue Growth: 12.5%
Purpose: Setting up a biodegradable packaging business
Jurisdiction: India
Industry: Sustainable packaging manufacturing
Collateral Type: Second charge over inventory
Estimated Market Value: INR 72000000
Estimated Realisable Value: INR 57500000
Indicative Coverage: 1.15x
Total Facility Amount: INR 50000000
Monthly Principal Instalment: INR 925926
Number of Instalments: 54"""
    result = DealReviewOrchestrator(tmp_path, Path("rules.json")).run(
        document_text=text,
        document_bytes=text.encode(),
        mode="Deterministic fallback",
        model_name="",
        test_id="PVC-GD-027",
    )

    failed_rules = {finding.rule_id for finding in result.agent1_result.findings if finding.status == "FAIL"}
    risk_ids = {flag.risk_id for flag in result.agent2_result.risk_flags}
    assert result.test_id == "PVC-GD-027"
    assert failed_rules == {"DEAL-002", "SEC-001"}
    assert risk_ids == set()
    assert result.agent2_result.overall_risk == "LOW"
    assert result.agent2_result.recommendation == "PROCEED"
    assert result.workflow_status == "INTERRUPTED"
