...
LABELS = {
    ...
    "term_years": ["term years", "loan term years", "tenor years"],
    "term_months": ["term months", "loan term months", "tenor months"],
...
def _currency(value: str | None, full_text: str) -> str | None:
    """Return the currency bound to the supplied monetary field.

    A labeled Loan Amount value is authoritative. Only fall back to the wider
    document when that label is absent, and then choose the earliest currency
    mention rather than the first currency in a hard-coded pattern list.
    """
    haystack = value if value else full_text[:2000]
...
def extract_deal_terms_model(text: str) -> DealTerms:
    ...
    term_years = _decimal(raw["term_years"])
    if term_years is None and term_value and re.search(r"years?|yrs?", term_value, re.I):
        term_years = term_number
    term_months = int(_decimal(raw["term_months"])) if _decimal(raw["term_months"]) is not None else None
    if term_months is None:
        term_months = _term_months(term_value)
    return DealTerms(
        ...
        loan_amount=loan_amount, term_years=term_years, term_months=term_months,
...
def _merge_explicit_extractions(
    primary: DealTerms, fallback: DealTerms,
) -> tuple[DealTerms, list[str], list[str]]:
    """Reconcile LLM output with values explicitly bound to document labels.

    The LLM remains useful for narrative and non-standard documents. When a
    recognized label is present, however, its parsed value is stronger evidence
    than an LLM value copied from a nearby financial line. This prevents Total
    Debt from being substituted for Loan Amount and prevents another line's
    currency from overwriting the facility currency.
    """
    ...
    for field, fallback_value in fallback_values.items():
        if fallback_value in (None, ""):
            continue
        if primary_values.get(field) in (None, ""):
            primary_values[field] = fallback_value
            filled.append(field)
        elif primary_values[field] != fallback_value:
            primary_values[field] = fallback_value
            corrected.append(field)
    return DealTerms.model_validate(primary_values), filled, corrected
...
def run_extractor(text: str, provider: str = "ollama", model_name: str = "gemma3:4b") -> ExtractionOutput:
    ...
    if provider == "ollama":
        ...
        terms, filled_fields, corrected_fields = _merge_explicit_extractions(terms, fallback_terms)
        if filled_fields:
            warnings.append("Explicit labelled values recovered for: " + ", ".join(filled_fields) + ".")
        if corrected_fields:
            warnings.append("LLM values corrected from explicit labelled evidence for: " + ", ".join(corrected_fields) + ".")
