...
def inject_styles() -> None:
    st.markdown(
        ...
        .hero p { color:#64748b; max-width:760px; margin:0; font-size:10px; }
...
def hero_markup() -> str:
    return (
        ...
        '<h1>Move from documents to decisions.</h1><p>Our four-agent workflow extracts, validates, checks policy compliance, '
...
def save_officer_decision(validation_id: str, decision: str | None) -> None:
    with sqlite3.connect(DB_PATH) as conn:
...
def workflow_timeline(states: dict[str, str], show_deal_officer: bool = False, officer_decision: str | None = None) -> str:
    labels = {
        "idle": "Ready", "in_progress": "In progress", "completed": "Completed", "failed": "Failed",
...
with new_tab:
        ...
        with st.container(border=True, height=250, key="new_validation_evaluation"):
            ...
            with officer_col:
                with st.container(key="deal_officer_panel"):
                    ...
                    officer_note = (
                        "Human decision required because an automated review returned an adverse outcome."
                        if officer_required else "Available only when Compliance Validator or Risk & Summary returns an adverse outcome."
                    )
                    ...
                    with action_approve:
                        approve_clicked = st.button("Approve", disabled=not officer_required, key=f"officer_action_approve_{displayed_validation_id}", width="stretch")
                    with action_reject:
                        reject_clicked = st.button("Reject", disabled=not officer_required, key=f"officer_action_reject_{displayed_validation_id}", width="stretch")
                    with action_hold:
                        hold_clicked = st.button("On Hold", disabled=not officer_required, key=f"officer_action_hold_{displayed_validation_id}", width="stretch")
                    ...
                    if selected_decision and displayed_validation_id:
                        ...
                        save_officer_decision(displayed_validation_id, selected_decision)
                    decision_text = officer_decision or ("Awaiting deal officer decision" if officer_required else "Not required")
                    decision_class = {
                        "Approve": "approve", "Reject": "reject", "On Hold": "on-hold",
                    }.get(officer_decision, "active" if officer_required else "")
                    st.markdown(
    ...
    if displayed_result and displayed_risk:
        workflow_states.update({"orchestrator": "completed", "extractor": "completed"})
    ...
    def update_workflow(stage: str | None = None, status: str | None = None) -> None:
        ...
        show_officer_step = (
            workflow_states["compliance_validator"] in {"failed", "non_compliant", "insufficient_data"}
            or workflow_states["risk_summary"] in {"failed", "adverse_risk"}
        )
    ...
    if run and uploaded:
        try:
            ...
            update_execution_status(f"Validation could not be completed: {exc}", "failed")
