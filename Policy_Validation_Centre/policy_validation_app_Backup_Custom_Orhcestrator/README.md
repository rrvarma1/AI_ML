...
```text
Orchestrator → Extractor ─┬→ Agent 1 Compliance ← rules.json
                         └→ Agent 2 Risk & Summary ← risk_rules.json
```

The Orchestrator starts after the user selects a document and clicks **Validate**. It invokes the Extractor first. After the Extractor creates the typed deal terms and JSON handoff, the Orchestrator dispatches Agent 1 and Agent 2 concurrently. Both agents receive the same extractor output and neither depends on the other agent's result.
